...
class MaskedAutoencoderViT(nn.Module):
    ...
    def strategy_masking(self, x, mask_ratio):
        ...
        for i in range(N):
            choosed_tks = []
            ids_shuffle_curr_img = []
            remain_tks = [i for i in range(L)]
            p = [1 for i in range(L)]
            while len(choosed_tks) < len_keep:
                norm_p = torch.tensor(p, dtype=torch.float64)
                norm_p = norm_p / norm_p.sum()
                choose = np.random.choice(remain_tks, 1, p=norm_p)
                idx = remain_tks.index(choose)
                remain_tks.remove(choose)
                del p[idx]
                choosed_tks.append(choose[0])
                ids_shuffle_curr_img.append(choose[0])
            ids_shuffle_curr_img.extend(remain_tks)
            ids_shuffle.append(ids_shuffle_curr_img)
            ids_keep.append(choosed_tks)

        # keep the first subset
        ids_keep=torch.tensor(ids_keep,device=x.device)
        x_masked = torch.gather(x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, D))

        ids_shuffle=torch.tensor(ids_shuffle,device=x.device)
    ...
    def strategy_masking_eff(self, x, mask_ratio):
        """
    ...
    def forward_encoder(self, x, mask_ratio):
        ...
        # x, mask, ids_restore = self.strategy_masking_eff(x, mask_ratio)
